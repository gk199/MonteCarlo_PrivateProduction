...
void HcalDigitizer::accumulateCaloHits(edm::Handle<std::vector<PCaloHit>> const &hcalHandle,
  ...
  if (isHCAL) {
    ...
    //    std::cout << "hcal hits orig ID = " ;
    //    for (unsigned int i = 0; i < hcalHitsOrig.size(); i++) std::cout << hcalHitsOrig[i].id() << ", ";
    //    std::cout << std::endl;
    std::vector<PCaloHit> hcalHits;
    hcalHits.reserve(hcalHitsOrig.size());

    //    std::cout << "test numbering = " << testNumbering_ << " force to false since scrambled det ID" << std::endl;
    // this was scrambling the detector ID when uncommented despite test numbering = false from config file (injected signal)
    ...
    for (unsigned int i = 0; i < hcalHitsOrig.size(); i++) {
      DetId id(hcalHitsOrig[i].id()); // unsigned int, made into det ID which is an unsigned int. Then prints the rawID of this det ID
      HcalDetId hid(id); 
      //      std::cout << "hcal hits orig ID before eliminating bad hits = " << hcalHitsOrig[i].id() << std::endl; // these are all same, but disagree with earlier hcalHitsOrig Id, determined to be from issue with test numbering variable set to true when config has it as false
    ...
    if (injectedHitsCells_.size() >= 4) {
      ...
      for (unsigned ic = 0; ic < injectedHitsCells_.size(); ic += 4) {
        if (ic + 4 > injectedHitsCells_.size())
          break;
	//	std::cout << "input parameters = " << injectedHitsCells_[ic] << injectedHitsCells_[ic+1] << injectedHitsCells_[ic+2] << injectedHitsCells_[ic+3] << " and first subdet = " << (HcalSubdetector)injectedHitsCells_[ic] << std::endl;
	//	std::cout << "HcalDetId() of parameters = " << HcalDetId((HcalSubdetector)injectedHitsCells_[ic],
	//								 injectedHitsCells_[ic + 1],
	//								 injectedHitsCells_[ic + 2],
	//								 injectedHitsCells_[ic + 3]) << std::endl;
